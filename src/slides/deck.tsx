...
function TitleSlide() {
  return (
    <SlideBase tone="blue">
      ...
      <div className="absolute left-[96px] top-[110px] rounded-2xl bg-white px-6 py-4">
        <img src={logo} alt="TM Pick n Pay" className="h-[54px] w-auto" />
      </div>
...
function GmvSlide() {
  return (
    <SlideBase tone="blue">
      ...
      <Body>
        <h2 className="slide-title">Monetising every stage of the cross-border basket</h2>
        ...
        <div className="mt-12 flex items-center gap-10 rounded-3xl bg-pnp-red p-12">
          <div>
            <span className="slide-kicker text-white/80">Annual gross merchandise value</span>
            <div className="mt-3 text-[112px] font-extrabold leading-none">US$61,200,000</div>
          </div>
          <p className="slide-body ml-auto max-w-[520px] text-white/85">
            Migrated from cash remittances and Malayitsha vans into a fully tracked, US$-settled
            retail pipeline.
          </p>
        </div>
      </Body>
      ...
      <Body>
        ...
        <div className="mt-10 grid grid-cols-3 gap-7">
          {[
            {
              ...
              d: "Spread captured on goods otherwise bought in South African cash-and-carries.",
              v: "US$12,852,000",
            },
            {
          ...
          <div className="flex flex-col justify-center rounded-3xl bg-pnp-blue p-9 text-white">
            <span className="slide-kicker text-pnp-gold">Total ecosystem</span>
            <div className="mt-4 text-[56px] font-extrabold leading-none">US$17,149,680</div>
            <p className="slide-caption mt-4 text-white/70">Projected annual revenue</p>
          </div>
        </div>
      </Body>
    </SlideBase>
...
function OptionCard({
  ...
  return (
    <div className="flex flex-col rounded-3xl border border-pnp-line bg-white p-9 shadow-sm">
      ...
      <div className="mt-6 grid grid-cols-3 gap-4 border-t border-pnp-line pt-6">
        {[
          ["Setup", setup],
          ["Monthly", opex],
          ["Revenue", revenue],
          ...
          <div key={k}>
            <div className="slide-chrome text-pnp-muted">{k}</div>
            <div className="slide-caption mt-1 font-bold text-pnp-ink">{v}</div>
          </div>
        ))}
      </div>
    </div>
...
function OptionsASlide() {
  return (
    <SlideBase>
      <SlideChrome kicker="Slide 05 · Business Model Options 1–2" index={6} />
      <Body>
        ...
        <div className="mt-10 grid grid-cols-2 gap-8">
          <OptionCard
            ...
            mechanics="We operate as a standalone entity mirroring your catalog via API. Senders pay us in US$; we buy stock at a negotiated wholesale discount and fulfil with our own driver network."
            setup="US$15,000"
            opex="US$8,500"
          ...
          <OptionCard
            ...
            setup="US$25,000"
            opex="US$2,000"
            revenue="US$40,000 + US$3,500/mo or 1.5–2% GMV"
          />
        </div>
...
function OptionsBSlide() {
  return (
    <SlideBase>
      <SlideChrome kicker="Slide 05 · Business Model Options 3–4" index={7} />
      <Body>
        <h2 className="slide-title text-pnp-blue">Partnership and hybrid structures</h2>
        <div className="mt-10 grid grid-cols-2 gap-8">
          <OptionCard
            n="Option 3"
            title="Tripartite Joint Venture"
            mechanics="PnP + Quatrohaus / Code Virtus + our firm. We provide the cross-border framework, payment tunnels and diaspora marketing; branch staff pick, third-party bike networks deliver."
            setup="US$5,000"
            opex="Absorbed in IT budget"
            revenue="60 / 20 / 20 net profit split"
          />
          <OptionCard
            n="Option 4"
            title="Programmatic Remittance Tokens"
            mechanics="Diaspora shoppers buy a US$ Eco-Voucher: auto-dispatch a staple hamper by bike courier, or push a secure barcode via SMS/WhatsApp for in-store redemption, bypassing cash-out queues."
            setup="US$10,000"
            opex="US$3,000"
            revenue="4–6% commission + float yield"
          />
        </div>
        <div className="mt-8 overflow-hidden rounded-3xl">
          <img
            src={bikeCourier}
            alt="Bike courier delivering groceries through the city"
            width={1600}
            height={1000}
            loading="lazy"
...
const MATRIX = {
  head: ["Metric", "1 · Reseller", "2 · White-Label", "3 · Joint Venture", "4 · Voucher Hybrid"],
  rows: [
...
function MatrixSlide() {
  return (
    <SlideBase tone="blue">
      <SlideChrome kicker="Slide 06 · Configuration Matrix" index={8} tone="blue" />
      <Body>
        <h2 className="slide-title">Commercial model comparison</h2>
        <div className="mt-12 overflow-hidden rounded-3xl bg-white/8">
          <div className="grid grid-cols-5 bg-pnp-red">
            {MATRIX.head.map((h) => (
              <div key={h} className="slide-body px-8 py-7 font-extrabold text-white">
                {h}
              </div>
            ))}
          </div>
          {MATRIX.rows.map((row, i) => (
            <div key={row[0]} className={`grid grid-cols-5 ${i % 2 ? "bg-white/5" : ""}`}>
              {row.map((cell, j) => (
                <div
                  key={cell + j}
                  className={`slide-caption px-8 py-7 ${j === 0 ? "font-bold text-pnp-gold" : "text-white/85"}`}
                >
                  {cell}
                </div>
              ))}
    ...
    <SlideBase tone="blue">
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <div className="rounded-2xl bg-white px-8 py-5">
          <img src={logo} alt="TM Pick n Pay" className="h-[56px] w-auto" />
        </div>
